...
export default function Answer({ label, originalText, setOriginal }) {
  ...
  if (isEditing) {
    return (
      <div className="mb-8">
        ...
        <div className="mt-4">
          <button onClick={() => {setEditing(false); setOriginal(text)}} className="px-4 py-2 rounded-full bg-green-200 text-green-800 font-semibold text-sm">Save Changes</button>
          <button onClick={() => {setEditing(false); setText(originalText)}} className="px-4 py-2 text-slate-600 font-semibold text-sm">Cancel</button>
        </div>
      </div>
    );
  } else {
