...
stateDiagram-v2
    ...
    filling --> sending : Submit, or Enter in a single-line box, with every required field filled
...
- **Enter in a single-line box also submits the form.** Enter in "TITLE", in a ShortAnswer "ANSWER" box, or in an integer or AIME box starts the browser's own form submission, as if "Submit" had been clicked. With other required fields still empty, this only shows the browser's bubble on the first of them and moves focus there. With everything else filled in, it submits the problem. Enter in "PROBLEM STATEMENT" or "SOLUTION" starts a new line.

> Technical note: the single-line editor saves on Enter but does not cancel the key's default action, so the browser's implicit form submission still happens.
...
| Keys                | Focus is already in "SOLUTION", so typing goes there.                                                                                                                                                                                                                                                                                             | Enter: closes "TITLE" or a ShortAnswer "ANSWER" and submits the form; submits from an integer or AIME box; new line in the statement and solution. Shift/Ctrl/Cmd+Enter: closes the statement or solution. Escape: abandons a reopened box. Tab: moves on, closing a box that holds text. |
...
- Enter in the title box with every other field filled submits the problem before the user has looked it over.
...
- Enter in a single-line box: a first local pass in headless Chromium confirmed that Enter in "TITLE" closes the box and starts the browser's submission, which the browser blocked because other fields were empty. That the problem is actually submitted when every other field is filled was not tried by hand. This looks like a bug: Enter in the title is there to close the box, and submitting the whole problem from it is easy to do by accident.
