...
- [Invites](invites.md#begin-editing) describes the wrong-domain button the same way: it attaches the new Google account to the user already signed in rather than switching users. Confirming it needs two real Google accounts.
...
- The login page accepts a path starting `/` followed by a tab character, and a first local pass confirmed the open redirect it allows: for a signed-in visitor, `/login?callbackUrl=%2F%09%2Fevil.example` answered with a redirect to `/<tab>/evil.example`, which the browser (headless Chromium) read as `//evil.example` and followed to `http://evil.example/`. This is a bug, and a security one: the check exists to stop exactly this.
