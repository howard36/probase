...
- **"Log in with Google"** (signed out, wrong domain) sends the browser to Google (see [sign-in](sign-in.md)). Google then returns the browser to this invite page, which runs its checks again for whichever account is now signed in. In the wrong-domain state the user is still signed in, so this does not switch users. If they pick a Google account Probase has never seen, it is attached to the Probase user already signed in: they stay the same user, but Probase now sees the new account's email, which passes the domain check, and from then on that Google account signs in as this user. If they pick a Google account that already belongs to another Probase user, the authentication library refuses with its own page ("To confirm your identity, sign in with the same account you used originally."). See [accounts and roles](../foundations/accounts-and-roles.md#the-session) and [signing in](sign-in.md).
...
| Network lost mid-request            | No effect until something is clicked. "Log in with Google" offline: if the first request fails the browser shows its own offline page; a failure at a later step does nothing visible. | "Something went wrong. Please try again." The page stays. The permission may still have been granted if the request arrived; a reload shows which.                                                                                 |
