...
| Testsolver type     | Serious testsolvers see it once their attempt is finished; Casual testsolvers see it at once. They cannot start an attempt as Casual, so they appear on it only with attempts made while Serious (which stay after switching), or one started from a locked page left open after switching. | A type changed elsewhere applies on the next refresh.        |
