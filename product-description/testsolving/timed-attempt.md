...
| Same record changed in another tab  | The same attempt can be open in two tabs, each with its own countdown and its own wrong-answer line.                                           | Solving or giving up in one tab leaves the other in the testsolving view until its countdown ends or it refreshes. Submitting there still counts: a wrong answer adds to the solved attempt's wrong answers, and a correct one moves its solve time later. |
